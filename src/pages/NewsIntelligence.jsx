...
function getTriggeredKeywords(title) {
  ...
  const NEGATIVE = ['crash','drop','bearish','ban','hack','loss','fall','fear','plunge','sell-off','lawsuit','collapse','decline','slump'];
  const lower = title.toLowerCase();
  return { positive: POSITIVE.filter(k => lower.includes(k)), negative: NEGATIVE.filter(k => lower.includes(k)) };
}
...
function SentimentGauge({ score }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <div style={{ position: 'relative', width: 80, height: 6, borderRadius: 3, overflow: 'hidden', background: 'linear-gradient(90deg, #ef4444 0%, #f59e0b 50%, #10b981 100%)' }}>
        <div style={{ position: 'absolute', top: -3, left: `${pct}%`, transform: 'translateX(-50%)', width: 5, height: 12, borderRadius: 2, background: '#fff', boxShadow: '0 0 4px rgba(255,255,255,0.8)' }} />
      </div>
      <span style={{ fontSize: 10, color, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace' }}>{score > 0 ? '+' : ''}{score.toFixed(2)}</span>
...
function MarketNarrative({ articles }) {
  const stats = useMemo(() => {
    ...
    const catCounts = {};
    articles.forEach(a => { const c = detectCategory(a.title); catCounts[c] = (catCounts[c] || 0) + 1; });
    ...
    <div style={{ background: `linear-gradient(135deg,${sc}08,#111827)`, border: `1px solid ${sc}25`, borderLeft: `3px solid ${sc}`, borderRadius: 12, padding: 18, marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
        {Object.entries(catCounts).sort((a, b) => b[1] - a[1]).map(([cat, count]) => {
          ...
          if (!cfg) return null;
          return <span key={cat} style={{ fontSize: 9, padding: '2px 7px', borderRadius: 4, background: `${cfg.color}14`, border: `1px solid ${cfg.color}28`, color: cfg.color, fontWeight: 600 }}>{cfg.icon} {cfg.label}: {count}</span>;
        })}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background: '#111827', border: `1px solid ${expanded ? cfg.color + '50' : '#1e2d45'}`, borderLeft: `3px solid ${cfg.color}`, borderRadius: 10, padding: '13px 15px', marginBottom: 8, transition: 'border-color 0.2s' }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 7, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setExpanded(v => !v)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569', fontSize: 11, padding: '0 2px', flexShrink: 0 }}>{expanded ? '▲' : '▼'}</button>
      </div>
...
function GdeltNewsCard({ article, rank }) {
  const sentColor = article.sentiment === 1 ? '#10b981' : article.sentiment === -1 ? '#ef4444' : '#f59e0b';
  const sentIcon = article.sentiment === 1 ? '▲' : article.sentiment === -1 ? '▼' : '◆';
  let timeAgo = '';
  try { timeAgo = formatDistanceToNow(new Date(article.published_at), { addSuffix: true }); } catch { timeAgo = ''; }
  return (
    <div style={{ background: '#111827', border: '1px solid #1e2d45', borderRadius: 10, padding: '14px 16px', marginBottom: 10, display: 'flex', gap: 14, alignItems: 'flex-start' }}>
      <div style={{ width: 28, height: 28, borderRadius: 6, flexShrink: 0, background: rank <= 2 ? 'rgba(59,130,246,0.15)' : '#0D1421', border: `1px solid ${rank <= 2 ? 'rgba(59,130,246,0.3)' : '#1e2d45'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: rank <= 2 ? '#60a5fa' : '#4a5568', fontFamily: 'JetBrains Mono, monospace' }}>{rank}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        {article.socialimage && <div style={{ marginBottom: 8, borderRadius: 6, overflow: 'hidden', maxHeight: 80 }}><img src={article.socialimage} alt="" style={{ width: '100%', height: 80, objectFit: 'cover', display: 'block' }} onError={e => { e.currentTarget.style.display = 'none'; }} /></div>}
        <a href={article.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}><div style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0', lineHeight: 1.5, marginBottom: 6 }}>{article.title}</div></a>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: '#60a5fa', background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 4, padding: '1px 6px' }}>{article.domain}</span>
          <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 4, fontWeight: 700, background: `${sentColor}14`, border: `1px solid ${sentColor}30`, color: sentColor, fontFamily: 'JetBrains Mono, monospace' }}>{sentIcon} {article.sentiment_label}</span>
          {timeAgo && <span style={{ fontSize: 10, color: '#4a5568', marginLeft: 'auto' }}>{timeAgo}</span>}
        </div>
      </div>
    </div>
  );
...
export default function NewsIntelligence() {
  ...
  const { data: gdeltArticles = [], isLoading: gdeltLoading, isFetching: gdeltFetching, isError: gdeltError, error: gdeltRawError, refetch: gdeltRefetch, dataUpdatedAt: gdeltUpdatedAt } = useGdeltNews('bitcoin crypto');
  const { data: instArticles = [], isLoading: instLoading, isFetching: instFetching, isError: instError, refetch: instRefetch, dataUpdatedAt: instUpdatedAt } = useGdeltNews('bitcoin ETF institutional SEC BlackRock Fidelity Grayscale adoption regulatory');
  ...
  const avgSentiment24h = instArticles.length > 0 ? parseFloat(((instBullishCount - instBearishCount) / instArticles.length).toFixed(2)) : 0;
  const sentColor = avgSentiment24h > 0.1 ? '#10b981' : avgSentiment24h < -0.1 ? '#ef4444' : '#f59e0b';

  if (!readModuleFlag('ENABLE_NEWS')) return <DisabledModuleBanner moduleName="ENABLE_NEWS" />;
      ...
      {mainTab === 'feed' && (
        <div style={{ maxWidth: 1000 }}>
          <div style={{ marginBottom: 16, padding: '12px 16px', background: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: 10, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            ...
            {!gdeltLoading && !gdeltError && <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: '#60a5fa', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 4, padding: '2px 8px' }}>{gdeltArticles.length} artigos</span>}
            <RefreshButton onRefresh={() => { gdeltRefetch(); }} isLoading={gdeltFetching} lastUpdated={gdeltUpdatedAt} label="Atualizar Feed Geral" />
          </div>
          {gdeltLoading && <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 0', gap: 12 }}><div style={{ width: 32, height: 32, borderRadius: '50%', border: '3px solid #1e2d45', borderTopColor: '#3b82f6', animation: 'spin 0.8s linear infinite' }} /><span style={{ fontSize: 13, color: '#475569' }}>Carregando notícias...</span><style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style></div>}
          {gdeltError && !gdeltLoading && <div style={{ padding: '32px 24px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, textAlign: 'center' }}><div style={{ fontSize: 13, color: '#ef4444', fontWeight: 600, marginBottom: 8 }}>Falha ao carregar notícias GDELT</div><div style={{ fontSize: 11, color: '#64748b', fontFamily: 'JetBrains Mono, monospace', marginBottom: 12 }}>{gdeltRawError?.message ?? 'Erro desconhecido'}</div><button onClick={() => gdeltRefetch()} style={{ padding: '6px 18px', borderRadius: 6, cursor: 'pointer', background: 'rgba(59,130,246,0.15)', color: '#60a5fa', border: '1px solid rgba(59,130,246,0.3)', fontSize: 11, fontWeight: 600 }}>Tentar novamente</button></div>}
          {!gdeltLoading && !gdeltError && gdeltArticles.length === 0 && !IS_LIVE && <div style={{ padding: '48px 24px', background: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 12, textAlign: 'center' }}><div style={{ fontSize: 20 }}>🛰️</div><div style={{ fontSize: 14, color: '#f59e0b', fontWeight: 700 }}>Modo MOCK ativo</div><div style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6, maxWidth: 380, margin: '0 auto' }}>Ative o modo <strong style={{ color: '#e2e8f0' }}>LIVE</strong> em Configurações para ver notícias reais via GDELT.</div></div>}
          {!gdeltLoading && !gdeltError && gdeltArticles.length === 0 && IS_LIVE && <div style={{ padding: '48px 24px', background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, textAlign: 'center' }}><div style={{ fontSize: 20 }}>🛰️</div><div style={{ fontSize: 14, color: '#ef4444', fontWeight: 700 }}>GDELT sem resultados</div><div style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6, maxWidth: 400, margin: '8px auto 16px' }}>A API pode estar temporariamente indisponível ou sem artigos no período. Verifique o painel de debug para detalhes do erro.</div><button onClick={() => gdeltRefetch()} style={{ padding: '6px 18px', borderRadius: 6, cursor: 'pointer', background: 'rgba(59,130,246,0.15)', color: '#60a5fa', border: '1px solid rgba(59,130,246,0.3)', fontSize: 11, fontWeight: 600 }}>Tentar novamente</button></div>}
          {!gdeltLoading && !gdeltError && gdeltArticles.length > 0 && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
                <div><div style={{ fontSize: 12, fontWeight: 700, color: '#60a5fa', marginBottom: 12 }}>Recentes — Parte 1</div>{gdeltArticles.slice(0, 12).map((a, i) => <GdeltNewsCard key={a.url} article={a} rank={i + 1} />)}</div>
              ...
              <div style={{ marginTop: 20, background: '#111827', border: '1px solid #1e2d45', borderRadius: 12, padding: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  <div style={{ flex: 1, textAlign: 'right', fontSize: 11, color: '#4a5568' }}>{gdeltArticles.length} artigos · via GDELT DOC 2.0</div>
                </div>
              </div>
            </>
          )}
            ...
            <div style={{ background: 'linear-gradient(135deg,#131e2e,#111827)', border: '1px solid #1e2d45', borderRadius: 14, padding: 18, marginBottom: 16 }}>
              ...
              {gdeltHistory.length > 0 ? (
                <div style={{ marginTop: 14 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', marginBottom: 8 }}>Histórico de Sentimento — últimos {gdeltHistory.length}d (Supabase)</div>
                  <ResponsiveContainer width="100%" height={90}>
                    <BarChart data={gdeltHistory} margin={{ top: 2, right: 4, left: -24, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" vertical={false} />
                      <XAxis dataKey="date" tick={{ fontSize: 8, fill: '#475569' }} tickLine={false} tickFormatter={v => v.slice(5)} />
                      <YAxis tick={{ fontSize: 8, fill: '#475569' }} tickLine={false} />
                      <Tooltip contentStyle={{ background: '#0d1421', border: '1px solid #2a3f5f', borderRadius: 6, fontSize: 10 }} formatter={(v, name) => [v, name === 'bullish' ? 'Bullish' : name === 'bearish' ? 'Bearish' : 'Neutro']} />
                  ...
                  <div style={{ fontSize: 9, color: '#334155', marginTop: 4 }}>Acumulado de {gdeltHistory.reduce((s, d) => s + d.total, 0)} artigos nos últimos {gdeltHistory.length} dias · Supabase gdelt_articles</div>
                </div>
              ) : <div style={{ fontSize: 9, color: '#334155', marginTop: 6 }}>💡 Histórico 7d acumulará após primeiros fetches live (gdelt_articles Supabase).</div>}
            </div>
          )}
          {instLoading && <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 0', gap: 12 }}><div style={{ width: 32, height: 32, borderRadius: '50%', border: '3px solid #1e2d45', borderTopColor: '#a78bfa', animation: 'spin 0.8s linear infinite' }} /><span style={{ fontSize: 13, color: '#475569' }}>Buscando notícias institucionais…</span><style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style></div>}
          {instError && !instLoading && <div style={{ padding: '24px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, textAlign: 'center' }}><div style={{ fontSize: 13, color: '#ef4444', fontWeight: 600, marginBottom: 8 }}>Falha ao carregar GDELT institucional</div><button onClick={() => instRefetch()} style={{ padding: '6px 18px', borderRadius: 6, cursor: 'pointer', background: 'rgba(59,130,246,0.15)', color: '#60a5fa', border: '1px solid rgba(59,130,246,0.3)', fontSize: 11, fontWeight: 600 }}>Tentar novamente</button></div>}
          {!instLoading && !instError && instArticles.length === 0 && !IS_LIVE && <div style={{ padding: '48px 24px', background: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 12, textAlign: 'center' }}><div style={{ fontSize: 20, marginBottom: 8 }}>🛰️</div><div style={{ fontSize: 14, color: '#f59e0b', fontWeight: 700 }}>Modo MOCK ativo</div><div style={{ fontSize: 12, color: '#64748b', marginTop: 6 }}>Ative o modo <strong style={{ color: '#e2e8f0' }}>LIVE</strong> em Configurações para ver notícias institucionais reais.</div></div>}
          {!instLoading && !instError && instArticles.length === 0 && IS_LIVE && <div style={{ padding: '48px 24px', background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 12, textAlign: 'center' }}><div style={{ fontSize: 20, marginBottom: 8 }}>🛰️</div><div style={{ fontSize: 14, color: '#ef4444', fontWeight: 700 }}>GDELT institucional sem resultados</div><div style={{ fontSize: 12, color: '#64748b', marginTop: 6, maxWidth: 420, margin: '8px auto 16px' }}>A API GDELT pode estar temporariamente indisponível. Tente novamente ou verifique o painel de debug.</div><button onClick={() => instRefetch()} style={{ padding: '6px 18px', borderRadius: 6, cursor: 'pointer', background: 'rgba(59,130,246,0.15)', color: '#60a5fa', border: '1px solid rgba(59,130,246,0.3)', fontSize: 11, fontWeight: 600 }}>Tentar novamente</button></div>}
          {!instLoading && !instError && instArticles.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              <div><div style={{ fontSize: 11, fontWeight: 700, color: '#a78bfa', marginBottom: 10 }}>Institucional — Parte 1</div>{instArticles.slice(0, Math.ceil(instArticles.length / 2)).map(a => <GdeltAICard key={a.url} article={a} />)}</div>
              <div><div style={{ fontSize: 11, fontWeight: 700, color: '#a78bfa', marginBottom: 10 }}>Institucional — Parte 2</div>{instArticles.slice(Math.ceil(instArticles.length / 2)).map(a => <GdeltAICard key={a.url} article={a} />)}</div>
            </div>
          )}
        </div>
      )}
