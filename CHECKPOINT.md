...
> Última atualização: 2026-05-15 (Sprint 7.4 — Page migrations batch C: remove mockData imports de Dashboard/ExecutiveReport/PredictivePanel/StablecoinFlow/ActionDashboard)
...
| **Sprint 7.4 — Page migrations batch C** | ✅ branch `claude/macro-page-fix-prompt-s6hoI` | **5 páginas limpas de mockData**. Dashboard.jsx: remove 10 imports mockData → `fmtNum`/`fmtPct` inline + 8 fallback consts (`BTC_FUTURES_FALLBACK`, `BTC_SPOT_FLOW_FALLBACK`, `MACRO_BOARD_FALLBACK`, `ON_CHAIN_FALLBACK`, `FEAR_GREED_FALLBACK`, `RECENT_ALERTS_FALLBACK`, `GLOBAL_RISK_FALLBACK`, `SOURCE_HEALTH_FALLBACK`, `AI_ANALYSIS_FALLBACK`). ExecutiveReport.jsx: remove 4 blocos de import mockData → 30 consts fallback inline; fix import ordering; fallbacks completos com todos os campos (BTC_FUTURES_MOCK_FALLBACK com ret_1d/ret_1w/funding_history/long_short_ratio; BTC_SOPR_FALLBACK com smoothed_7d; BTC_EXCHANGE_NETFLOW_FALLBACK com exchange_reserves/netflow_7d; STABLECOIN_SNAPSHOT_FALLBACK com usdt/usdc/sigma_vs_7d; ETF_FLOWS_FALLBACK com funds[]; LIQUIDATION_CLUSTERS_FALLBACK com total_shorts_at_risk_10pct; BTC_NUPL_FALLBACK history como objeto). PredictivePanel.jsx: remove 4 imports mockDataPredictive → 4 consts fallback (SCENARIOS_24H_FALLBACK com 5 cenários base SPOT_FALLBACK=84K, BREAKOUT_TABLE_FALLBACK=[], INSTITUTIONAL_PRESSURE_FALLBACK, PRICE_PATHS_FALLBACK). StablecoinFlow.jsx: remove mockDataStablecoin → 7 consts fallback. ActionDashboard.jsx: remove mockDataActionDashboard + mockData (globalRisk/fearGreed/btcFutures) → 7 consts fallback. Build ✅ |
