...
> Última atualização: 2026-05-16 (PR #106 merged — cache headers movidos para render.yaml; fix P1 Codex)
...
| **Cache headers Render — fix P1 Codex** | ✅ PR #106 MERGEADO | `public/_headers` (Netlify) removido; regras movidas para `render.yaml` campo `headers` (Blueprint Render): `/index.html` → `no-cache, no-store, must-revalidate`; `/assets/*` → `public, max-age=31536000, immutable`. Corrige "Importing a module script failed" após deploy. |
